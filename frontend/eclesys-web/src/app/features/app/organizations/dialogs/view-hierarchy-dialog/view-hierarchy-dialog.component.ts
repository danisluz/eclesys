...
export class ViewHierarchyDialogComponent implements OnInit {
  ...
  @Input() rootOrganizationId: string | undefined;
  ...
  private updateFilterLabels(): void {
    this.filterOptions = [
      { label: 'Todos', value: 'all' },
      { label: this.churchUnit?.sectorLabel || 'Setores', value: 'SECTOR' },
      { label: this.churchUnit?.congregationLabel || 'Congregações', value: 'CONGREGATION' },
    ];
  }
  ...
  private async loadHierarchy(): Promise<void> {
    ...
    try {
      ...
      const findChurch = (units: OrganizationUnit[]): OrganizationUnit | null => {
      ...
      if (this.rootOrganizationId) {
        const findUnitById = (units: OrganizationUnit[], id: string): OrganizationUnit | null => {
          for (const unit of units) {
        ...
        const targetUnit = findUnitById(hierarchy, this.rootOrganizationId);
        if (targetUnit) hierarchy = [targetUnit];
      }

      if (hierarchy.length > 0) {
        this.allTreeNodes = await Promise.all(hierarchy.map((u) => this.buildTreeNode(u)));
        this.applyFilter();
      }
    } catch (error) {
  ...
  private async buildTreeNode(unit: OrganizationUnit): Promise<UITreeNode<HierarchyNodeData>> {
    let roles: Array<{ memberName: string; roleName: string }> = [];
    try {
      const assignments = await firstValueFrom(this.rolesService.listRoles(unit.id));
      roles = assignments.map((a) => ({ memberName: a.userName, roleName: a.functionRoleName }));
    } catch {
      roles = [];
    }

    const children = unit.children
      ? await Promise.all(unit.children.map((child) => this.buildTreeNode(child)))
      : [];

    return { label: unit.name, data: { unit, roles }, children, expanded: true };
  }
  ...
  applyFilter(): void {
    if (this.selectedFilter === 'all') {
      ...
      this.filteredTreeNodes = this.filterNodes(this.allTreeNodes, this.selectedFilter);
    }
  }

  private filterNodes(nodes: UITreeNode<HierarchyNodeData>[], type: string): UITreeNode<HierarchyNodeData>[] {
  ...
  getTypeLabel(type: string): string {
    ...
    if (type === 'CONGREGATION') return this.churchUnit?.congregationLabel || 'Congregação';
    return type;
  }
